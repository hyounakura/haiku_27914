...
## 目指した課題解決
俳句を詠んでデータに保存させる際にgetCurrentPositionメソッドで現在地を一緒に保存させるようにし、また現在地を定期的に取得するためにwatchPositionメソッドを使い、現在地の近くで詠まれた俳句を取得できるようにしました。取得する距離の範囲をユーザーが指定できるように実装しました。
